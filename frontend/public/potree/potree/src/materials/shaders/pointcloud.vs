...
uniform float uFilterIds[32];
uniform int uFilterIdsCount;
uniform float uGpsRawScale;   // potree.scale = 1/(amax-amin) for GPS time decompression
uniform float uGpsRawOffset;  // potree.offset = amin for GPS time decompression
...
// Maps an integer ID to a visually distinct hue using a hash function
vec3 colorFromId(float id) {
	// Hash function to map id -> hue in [0, 1)
	float h = fract(sin(id * 127.1 + 311.7) * 43758.5453);
	// HSV to RGB: S=0.8, V=0.9 for vivid but not garish colors
	float s = 0.80;
	float v = 0.90;
...
float getFilterValue(){
	...
	#if defined(color_type_source_id) || defined(color_type_point_source_id)
		value = pointSourceID;
	#elif defined(color_type_classification)
		value = classification;
	#elif defined(color_type_gps_time)
		value = gpsTime;
	#elif defined(color_type_intensity) || defined(color_type_intensity_id)
		value = intensity;
	#elif defined(color_type_return_number)
		value = returnNumber;
	#elif defined(color_type_extra)
		value = aExtra;
	#else
		// Fallback: try pointSourceID
		value = pointSourceID;
	#endif
...
vec3 getColor(){
	vec3 color;
		...
		color = getRGB();
	#elif defined(color_type_world_position)
		vec4 world = modelMatrix * vec4( position, 1.0 );
		color = world.xyz;
	#elif defined color_type_coordinate_x
		vec4 world = modelMatrix * vec4( position, 1.0 );
		float w = (world.x - elevationRange.x) / (elevationRange.y - elevationRange.x);
		color = texture2D(gradient, vec2(w,1.0-w)).rgb;
	#elif defined color_type_coordinate_y
		vec4 world = modelMatrix * vec4( position, 1.0 );
		float w = (world.y - elevationRange.x) / (elevationRange.y - elevationRange.x);
		color = texture2D(gradient, vec2(w,1.0-w)).rgb;
	#elif defined color_type_segment_id
		color = colorFromId(aExtra);
		...
		color = colorFromId(pointSourceID);
	#elif defined color_type_point_source_id
		color = colorFromId(pointSourceID);
	#elif defined color_type_gps_id
		color = colorFromId(gpsTime);
	#elif defined color_type_intensity_id
		color = colorFromId(intensity);
...
void doClipping(){
	if(uFilterIdsCount > 0){
		bool matched = false;

		// We check multiple attributes to make segmentation robust across different color modes.
		// Even if coloring by RGB, these attributes will be force-bound by the renderer.
		float v1 = intensity;
		float v2 = pointSourceID;
		float v3 = classification;

		for(int i = 0; i < 32; i++){
			if(i >= uFilterIdsCount) break;
			float tid = uFilterIds[i];
			if(abs(v1 - tid) < 1.5 || abs(v2 - tid) < 1.5 || abs(v3 - tid) < 1.5) {
				matched = true;
				break;
			}
		}
