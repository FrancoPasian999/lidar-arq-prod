...
// Normalized lookup map (all lowercase, no spaces)
let normalizedAttributeLocations = {
	"position": 0,
	"color": 1,
	"rgba": 1,
	"intensity": 2,
...
class Shader {
	...
	linkProgram() {
		...
		if (cached) {
			...
			// console.log("[SHADER ATTRIB LOCATIONS]", JSON.stringify(this.attributeLocations));
	...
	setUniform1i(name, value) {
		let gl = this.gl;
		let location = this.uniformLocations[name] || this.uniformLocations[name + "[0]"];

		if (location == null) {
			if (name.startsWith("uFilter")) {
	...
	setUniform1fv(name, value) {
		...
		if (name === "uFilterIds") {
			// console.log(`GPU PUSH [${name}]:`, value.slice(0, 5));
		}
...
export class Renderer {
	...
	renderNodes(octree, nodes, visibilityTextureData, camera, target, shader, params) {
		...
		for (let node of nodes) {
			...
			let globalSeg = window.Potree && window.Potree.segmentation;
			let count = (globalSeg && globalSeg.active) ? globalSeg.filterIdsCount : 0;
			let ids = globalSeg ? globalSeg.filterIds : null;
			
			// Only if global is inactive, check material (fallback)
			if (count === 0 && material.uniforms.uFilterIdsCount) {
				count = material.uniforms.uFilterIdsCount.value;
				if (material.uniforms.uFilterIds) {
					ids = material.uniforms.uFilterIds.value;
				}
			}

			if (count > 0 && ids) {
				shader.setUniform1fv("uFilterIds", ids);
				shader.setUniform1i("uFilterIdsCount", count);

			} else {
				shader.setUniform1i("uFilterIdsCount", 0);
			}
			...
			if(geometry.attributes["gps-time"]){
				...
				// Raw decompression parameters for ID comparison in doClipping()
				// real_value = compressed / potree.scale + potree.offset
				shader.setUniform1f("uGpsRawScale", bufferAttribute.potree.scale || 0.0);
				shader.setUniform1f("uGpsRawOffset", bufferAttribute.potree.offset || 0.0);
			...
			if (i === 0 && !auditLogged) {
				// console.log("[BRANCH] isExtraAttribute:", isExtraAttribute, 
				// 	"| activeAttrib:", material.activeAttributeName,
				// 	"| geomKeys:", Object.keys(geometry.attributes));
			}
			...
			if(isExtraAttribute){
				...
				// === UNIVERSAL SEGMENTATION FIX ===
				// Ensure that the attributes needed for filtering are always bound, 
				// even if the shader is primarily rendering another mode (like RGB).
				for (const attrName in geometry.attributes) {
					const normKey = attrName.toLowerCase().replace(/[^a-z0-9]/g, "");
					const vboAttr = webglBuffer.vbos.get(attrName);
					if (!vboAttr) continue;
					const bufAttr = geometry.attributes[attrName];
					let targetLoc = -1;

					if (normKey === "intensity" || normKey === "intensityid") {
						targetLoc = attributeLocations["intensity"] ? attributeLocations["intensity"].location : 2;
					} else if (normKey === "sourceid" || normKey === "pointsourceid") {
						targetLoc = attributeLocations["pointSourceID"] ? attributeLocations["pointSourceID"].location : 6;
					} else if (normKey === "classification") {
						targetLoc = attributeLocations["classification"] ? attributeLocations["classification"].location : 3;
					}

					if (targetLoc !== -1) {
						const t = this.glTypeMapping.get(bufAttr.array.constructor);
						gl.bindBuffer(gl.ARRAY_BUFFER, vboAttr.handle);
						gl.vertexAttribPointer(targetLoc, bufAttr.itemSize, t, bufAttr.normalized, 0, 0);
						gl.enableVertexAttribArray(targetLoc);
					}
				}
				// === END UNIVERSAL SEGMENTATION FIX ===
				...
				if (!window._segDebug) {
					window._segDebug = true;
					// console.log("[SEG_DEBUG] Geometry attribute names:", Object.keys(geometry.attributes));
					const psid = geometry.attributes["point source id"];
					if (psid) {
						// console.log("[SEG_DEBUG] point source id - normalized:", psid.normalized, "| first 5 values:", Array.from(psid.array.slice(0, 5)));
					} else {
						// console.warn("[SEG_DEBUG] NO 'point source id' attribute found!");
						for (const k in geometry.attributes) {
							const a = geometry.attributes[k];
							// console.log(`  [SEG_DEBUG] attr "${k}" first val:`, a.array[0]);
						}
					}
				}
				...
				for (const attributeName in geometry.attributes) {
					...
					let attributeLocation = undefined;
					if (attributeLocations[attributeName] !== undefined) {
						attributeLocation = attributeLocations[attributeName].location;
					} else {
						// Case-insensitive fallback
						let normName = attributeName.toLowerCase().replace(/[^a-z0-9]/g, "");
						if (normalizedAttributeLocations[normName] !== undefined) {
							attributeLocation = normalizedAttributeLocations[normName];
						}
					}

					if (attributeLocation !== undefined && vbo !== undefined) {
						if (!auditLogged) {
							// console.log(`[BIND] Mapping attribute "${attributeName}" to shader location ${attributeLocation}`);
						}
