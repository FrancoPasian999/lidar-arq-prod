...
export class TransformationTool {
	constructor(viewer) {
		...
		this.translationHandles = {
			...
			"translation.z_plane": {name: "translation.z_plane", node:  new THREE.Object3D(), color: 0x00ffff, alignment: [0, 0, 1]},
		};
	...
	initializeTranslationHandles(){
		...
		let torusGeometry = new THREE.TorusGeometry(1, 0.1, 8, 64);
		...
		for(let handleName of Object.keys(this.translationHandles)){
			...
			let isZPlane = (handleName === "translation.z_plane");
			let box;
			if (isZPlane) {
				box = new THREE.Mesh(torusGeometry, material);
				box.scale.set(6, 6, 6);
			} else {
			...
			let pickVolume;
			if (isZPlane) {
				pickVolume = new THREE.Mesh(torusGeometry, pickMaterial);
				pickVolume.scale.set(1.5, 1.5, 5);
			} else {
				pickVolume = new THREE.Mesh(boxGeometry, pickMaterial);
	...
	update () {
		...
		if(this.selection.length === 1){
            ...
            const isPlane = selected.isPlaneToolInstance;
            this.scaleHandles["scale.z+"].node.visible = !isPlane;
            this.scaleHandles["scale.z-"].node.visible = !isPlane;
            this.translationHandles["translation.z"].node.visible = !isPlane;
            this.translationHandles["translation.z_plane"].node.visible = !!isPlane;

            if (isPlane) {
                this.translationHandles["translation.z_plane"].node.position.set(0, 0, 0.8);
            }
