...
export class Measure extends THREE.Object3D {
	constructor () {
		...
		this.heightHorizontalLabel = createHeightLabel();
		...
		this.add(this.heightHorizontalLabel);
	...
	update () {
		...
		{ // update height stuff
			...
			if(heightEdge) heightEdge.visible = this.showHeight;
			if(this.heightLabel) this.heightLabel.visible = this.showHeight;
			if(this.heightHorizontalLabel) this.heightHorizontalLabel.visible = this.showHeight;
			...
			if (this.showHeight) {
				...
				if (this.heightHorizontalLabel) {
					this.heightHorizontalLabel.position.copy(horizontalLabelPosition);
					
					let distanceXY = lowPoint.distanceTo(start);
					if(this.lengthUnit != null && this.lengthUnitDisplay != null){
						distanceXY = distanceXY / this.lengthUnit.unitspermeter * this.lengthUnitDisplay.unitspermeter;
					}
					let txtDistanceXY = Utils.addCommas(distanceXY.toFixed(2));
					this.heightHorizontalLabel.setText(`${txtDistanceXY} ${suffix}`);
				}
			}
		}
