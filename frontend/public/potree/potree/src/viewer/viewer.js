...
console.log('Potree Segmentation Tool v2.5.0 Active');
...
import {PlanesTool} from "../utils/PlanesTool.js";
...
export class Viewer extends EventDispatcher{
	...
	constructor(domElement, args = {}){
		...
		this.sessionId = "sess_" + new Date().toISOString().replace(/[:.]/g, "-").slice(0, 19);
		console.log(`Potree Viewer Initialized - Session ID: ${this.sessionId}`);
		...
		this.planesTool = new PlanesTool(this);

		this.renderArea.addEventListener('mouseup', (e) => {
			if (e.shiftKey && this.segmentationWindow && this.segmentationToolActive) {
				...
				if (intersection && intersection.point) {
					...
					const pc = intersection.pointcloud;
					const attrName = pc.material.activeAttributeName;

					// The picker stores attributes on pt using the exact buffer key names (from geometry.attributes).
					// These match the loader names: "point source id" (with spaces), "intensity", etc.
					const attrMap = {
						"source_id":       ["point source id", "pointSourceID", "source id"],
						"point_source_id": ["point source id", "pointSourceID", "source id"],
						"gps_id":          ["gps-time", "gpsTime"],
						"gps_time":        ["gps-time", "gpsTime"],
						"intensity_id":    ["intensity", "Intensity"],
						"intensity":       ["intensity", "Intensity"],
						"intensity_gradient": ["intensity", "Intensity"],
						"classification":  ["classification"],
						"rgba":            ["rgba", "rgb", "RGB", "RGBA"],
						"rgb":             ["rgba", "rgb", "RGB", "RGBA"],
						"elevation":       ["elevation", "position_z"],
						"height":          ["elevation", "position_z"],
					};

					const internalNames = attrMap[attrName];
					if (internalNames) {
						let pickedValue = undefined;
						for (const name of internalNames) {
							const raw = pt[name];
							if (raw !== undefined) {
								pickedValue = (raw.length !== undefined) ? raw[0] : raw;
								break;
							}
						}
						if (pickedValue !== undefined) {
							this.segmentationWindow.addId(pickedValue);
						}
					}
				}
			}
	...
	async capturePositionData() {
		...
		const oldEDL = this.getEDLEnabled();
		const visiblePointClouds = this.scene.pointclouds.filter(pc => pc.visible);
		const oldAttributes = visiblePointClouds.map(pc => pc.material.activeAttributeName);

		this.setEDLEnabled(false);
		for (let pc of visiblePointClouds) {
			pc.material.activeAttributeName = "world_position";
		}

		renderer.setRenderTarget(target);
		renderer.clear(true, true, true);
		
		this.pRenderer.render(this.scene.scenePointCloud, this.scene.getActiveCamera(), target);

		const pixels = new Float32Array(width * height * 4);
		renderer.readRenderTargetPixels(target, 0, 0, width, height, pixels);

		this.setEDLEnabled(oldEDL);
		for (let i = 0; i < visiblePointClouds.length; i++) {
			visiblePointClouds[i].material.activeAttributeName = oldAttributes[i];
		}
	...
	isPointVisible (pointData, index, worldPos, pointcloud, skipClipping = false) {
		const scene = this.scene;
		const material = pointcloud.material;

		if (pointData.classification) {
			const classID = pointData.classification[index];
			const classValue = material.classification[classID];
			if (classValue && !classValue.visible) return false;
		}

		if (window.Potree && window.Potree.segmentation && window.Potree.segmentation.active) {
			const filterIds = window.Potree.segmentation.filterIds;
			const count = window.Potree.segmentation.filterIdsCount;
			if (count > 0) {
				let matched = false;
				const v1 = pointData.intensity ? pointData.intensity[index] : -1;
				const v2 = pointData.pointSourceID ? pointData.pointSourceID[index] : -1;
				const v3 = pointData.classification ? pointData.classification[index] : -1;
				for (let i = 0; i < count; i++) {
					if (Math.abs(filterIds[i] - v1) < 1.5 || Math.abs(filterIds[i] - v2) < 1.5 || Math.abs(filterIds[i] - v3) < 1.5) {
						matched = true; break;
					}
				}
		...
		const clipVolumes = scene.volumes.filter(v => v.clip);
		if (!skipClipping && clipVolumes.length > 0 && scene.clipTask !== ClipTask.NONE && scene.clipTask !== ClipTask.HIGHLIGHT) {
			let insideCount = 0;
			for (let volume of clipVolumes) {
				if (volume.containsPoint && volume.containsPoint(worldPos)) insideCount++;
			}
			let isInside = (scene.clipMethod === ClipMethod.INSIDE_ANY) ? (insideCount > 0) : (insideCount === clipVolumes.length);
			if (scene.clipTask === ClipTask.SHOW_INSIDE && !isInside) return false;
			if (scene.clipTask === ClipTask.SHOW_OUTSIDE && isInside) return false;
		}

		if (pointData.returnNumber && this.filterReturnNumberRange) {
			const val = pointData.returnNumber[index];
			if (val < this.filterReturnNumberRange[0] || val > this.filterReturnNumberRange[1]) return false;
		}
		if (pointData.numberOfReturns && this.filterNumberOfReturnsRange) {
			const val = pointData.numberOfReturns[index];
			if (val < this.filterNumberOfReturnsRange[0] || val > this.filterNumberOfReturnsRange[1]) return false;
		}

		return true;
	}

	async exportVisiblePointsMetadata () {
		const recipe = {
		...
		// 1. Point Cloud Data
		for (const pc of this.scene.pointclouds) {
			if (!pc.visible) continue;
			
			// Fetch original path if it exists (linked sidecar)
			let sourceLas = null;
			try {
				const baseUrl = pc.pcoGeometry.url.substring(0, pc.pcoGeometry.url.lastIndexOf('/'));
				const response = await fetch(`${baseUrl}/source_path.txt`);
				if (response.ok) {
					sourceLas = (await response.text()).trim();
				}
				
				// Fallback: check if source.las exists directly in the folder
				if (!sourceLas) {
					const resHead = await fetch(`${baseUrl}/source.las`, { method: 'HEAD' });
					if (resHead.ok) {
						sourceLas = "source.las";
					}
				}
			} catch (e) {
				console.log("No source path information found for this cloud.");
			}


			recipe.pointclouds.push({
				name: pc.name,
				source_las: sourceLas,
				offset: [pc.pcoGeometry.offset.x, pc.pcoGeometry.offset.y, pc.pcoGeometry.offset.z],
				matrixWorld: pc.matrixWorld.elements,
				classification: Object.fromEntries(
					Object.entries(pc.material.classification).map(([k, v]) => [k, v.visible])
				)
			});
		}

		// 2. Clipping Volumes
		const clipVolumes = this.scene.volumes.filter(v => v.clip);
		for (const v of clipVolumes) {
			recipe.clipVolumes.push({
				name: v.name,
				type: v instanceof BoxVolume ? "Box" : (v instanceof SphereVolume ? "Sphere" : "Unknown"),
				matrixWorld: v.matrixWorld.elements,
				clipTask: this.clipTask,
				clipMethod: this.clipMethod
			});
		}

		// 3. Segmentation IDs
		if (this.segmentationWindow && this.segmentationWindow.ids.size > 0) {
			const activeIds = Array.from(this.segmentationWindow.ids).map(Number);
			recipe.segmentation.count = activeIds.length;
			recipe.segmentation.filterIds = activeIds;
		}
		...
		a.href = url;
		a.download = `recipe_${this.sessionId}_${new Date().getTime()}.json`;
		a.click();
