...
import { SegmentationWindow } from "./SegmentationWindow.js";
import { ScreenshotWindow } from "./ScreenshotWindow.js";
import { PlanesWindow } from "./PlanesWindow.js";
...
export class Sidebar {
	...
	initToolbar() {
		...
		{ // Color tools
			let elColor = $('#color_tools');

			const colorButtons = [
				{
					label: 'X',
					title: 'Color_X: Color by X coordinate',
					action: () => {
						for (let pc of this.viewer.scene.pointclouds) {
							let material = pc.material;
							let bbox = pc.getBoundingBoxWorld();
							material.elevationRange = [bbox.min.x, bbox.max.x];
							material.activeAttributeName = "coordinate_x";
						}
					}
				},
				{
					label: 'Y',
					title: 'Color_Y: Color by Y coordinate',
					action: () => {
						for (let pc of this.viewer.scene.pointclouds) {
							let material = pc.material;
							let bbox = pc.getBoundingBoxWorld();
							material.elevationRange = [bbox.min.y, bbox.max.y];
							material.activeAttributeName = "coordinate_y";
						}
					}
				},
				{
					label: 'Z',
					title: 'Color_Z: Color by Z coordinate',
					action: () => {
						for (let pc of this.viewer.scene.pointclouds) {
							let material = pc.material;
							let bbox = pc.getBoundingBoxWorld();
							material.elevationRange = [bbox.min.z, bbox.max.z];
				...
				{
					label: '\u21BA',
					title: 'Color_Reset: Restore original RGB color',
					action: () => {
						for (let pc of this.viewer.scene.pointclouds) {
							let material = pc.material;
							const rgbAttr = ["rgba", "rgb", "RGB", "RGBA"].find(name =>
								pc.getAttribute(name) !== null
							);
							material.activeAttributeName = rgbAttr || "rgba";
						}
					}
				},
				{
					label: 'Src',
					title: 'Color by source_id (each ID gets a unique color)',
					action: () => {
						let found = false;
						for (let pc of this.viewer.scene.pointclouds) {
							const srcAttr = pc.getAttribute("source_id") || 
								pc.getAttribute("point_source_id") || 
								pc.getAttribute("gps_time") ||
								pc.getAttribute("intensity") ||
								pc.getAttribute("Intensity");
							if (srcAttr) {
								found = true;
								if (srcAttr.name === "gps_time") {
									pc.material.activeAttributeName = "gps_id";
								} else if (srcAttr.name.toLowerCase() === "intensity") {
									pc.material.activeAttributeName = "intensity_id";
								} else {
									pc.material.activeAttributeName = "source_id";
								}
							}
						}
						if (!found) {
							this.viewer.postMessage(
								"No 'source_id', 'gps_time' or 'intensity' attribute found in the loaded point cloud(s).",
								{ duration: 4000 }
							);
						}
					}
				...
				{
					label: 'Cl',
					title: 'Color by Classification (uses colors from the Classification panel)',
					action: () => {
						for (let pc of this.viewer.scene.pointclouds) {
			...
			for (let btn of colorButtons) {
				...
				let icon = $('<img>').attr('src', Potree.resourcePath + '/icons/volume.svg')
					.css({ width: '32px', height: '32px' })
					.addClass('button-icon');

				let label = $('<span>' + btn.label + '</span>').css({
					fontSize: '9px',
					color: '#ccc',
					marginTop: '2px',
					fontWeight: 'bold',
				...
				wrapper.click(btn.action);
				elColor.append(wrapper);
			}
		}

		{
			this.viewer.segmentationWindow = new SegmentationWindow(this.viewer);
			const elSegmentation = $('#segmentation_tools');

			const btn = {
				label: 'Segment',
				title: 'Open segmentation control window',
				action: () => {
					this.viewer.segmentationWindow.show();
				}
			};
			...
			let icon = $('<img>').attr('src', Potree.resourcePath + '/icons/clip-polygon.svg')
				.css({ width: '32px', height: '32px' })
				.addClass('button-icon');
			...
			wrapper.append(icon).append(label);
			wrapper.click(btn.action);
			elSegmentation.append(wrapper);
		}

		{
			this.viewer.planesWindow = new PlanesWindow(this.viewer);
			const elSegmentation = $('#segmentation_tools');

			const btn = {
				label: 'Planes',
				title: 'Open planes tool',
				action: () => {
					this.viewer.planesWindow.show();
				}
			};
		...
		{
			this.viewer.screenshotWindow = new ScreenshotWindow(this.viewer);
			const elSegmentation = $('#segmentation_tools');

			const btn = {
				label: 'Screenshots',
				title: 'Open screenshots and data export window',
				action: () => {
					this.viewer.screenshotWindow.show();
				}
			};
			...
			let icon = $('<img>').attr('src', Potree.resourcePath + '/icons/perspective-camera.svg')
				.css({ width: '32px', height: '32px' })
				.addClass('button-icon');
