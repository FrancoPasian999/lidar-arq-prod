...
export class PointCloudOctree extends PointCloudTree {
	...
	toTreeNode (geometryNode, parent) {
		...
		sceneNode.onBeforeRender = (_this, scene, camera, geometry, material, group) => {
			if (material.program) {
				...
				if (material.program.getUniforms().map.uFilterIds) {
					material.program.getUniforms().map.uFilterIds.setValue(_this.getContext(), material.uniforms.uFilterIds.value);
				}

				if (material.program.getUniforms().map.uFilterIdsCount) {
					material.program.getUniforms().map.uFilterIdsCount.setValue(_this.getContext(), material.uniforms.uFilterIdsCount.value);
				}
			}
		};
	...
	pick(viewer, camera, ray, params = {}){
		...
		{ // update pick material
			...
			pickMaterial.uniforms.uFilterIds.value = this.material.uniforms.uFilterIds.value;
			pickMaterial.uniforms.uFilterIdsCount.value = this.material.uniforms.uFilterIdsCount.value;
